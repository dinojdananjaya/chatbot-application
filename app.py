...
def get_response():
    ...
    if 'awaiting_user_id' in session and session['awaiting_user_id']:
        ...
        # Match the user name if present
        ...
        if name_match:
            ...
            responses = {
                ...
                "farewell": "Goodbye! Have a great day!" if 'user_name' not in session else f"Goodbye {session['user_name']}! Have a great day!",
                "gratitude": "You're welcome!" if 'user_name' not in session else f"You're welcome, {session['user_name']}!"
            }
